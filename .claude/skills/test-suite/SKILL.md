...
Execute the full test suite and fix any failures. When fixing failures, follow the project's fixing workflow: discover all failures, create a task list, fix one test at a time, then run full suite to confirm.

## Process

1. Run `npm test` to execute the test suite (Jest in utils packages, Vitest where configured)
