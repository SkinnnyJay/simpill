...
Resolve all linting and formatting issues across the project. When fixing issues, follow the project's fixing workflow: run lint to discover all issues, create a task list, fix one at a time, then run full lint to confirm.
...
- Never add lint-disable comments if there's a better fix
- Don't add unnecessary comments -- make them count
- Use Biome for formatting: see project biome.json (e.g. 2-space indent, double quotes, semicolons)

## Pass Criteria

- `npm run lint` exits with code 0
- `npm run check:fix` or `npm run format` (Biome) applied; all files conform to project rules
