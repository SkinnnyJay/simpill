...
const DEFAULT_DISPLAY_NAME = "SafeContext";
...
export function createSafeContext<T>(displayName?: string): SafeContextResult<T> {
  ...
  function useCtx(): T {
    ...
    if (value === null) {
      throw new Error(`[${name}] useCtx must be used within the corresponding Provider.`);
    }
    return value;
  }
...
export function useSafeContext<T>(context: Context<T | null>): T {
  ...
  if (value === null) {
    throw new Error("useSafeContext must be used within the corresponding Provider.");
  }
  return value;
}
