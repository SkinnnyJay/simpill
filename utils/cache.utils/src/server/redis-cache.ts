...
export class RedisCache<V> {
  ...
  async set(key: string, value: V, ttlMs?: number): Promise<void> {
    if (value === undefined) {
      throw new TypeError("RedisCache does not support storing undefined");
    }
    const k = this.toKey(key);
    const serialized = JSON.stringify(value);
    if (typeof serialized !== "string") {
      throw new TypeError("RedisCache serialization produced non-string value");
    }
  ...
  async get(key: string): Promise<V | undefined> {
    ...
    try {
      ...
      throw new Error(`RedisCache: invalid JSON for key ${key}: ${message}`);
    }
  }
