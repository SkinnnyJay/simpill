...
- **CircuitBreaker(options)** — run(fn), getState(): 'closed' | 'open' | 'half-open'
...
There are **no** built-in metrics or event callbacks. To observe the breaker, **poll getState()** or wrap **run()** (e.g. log on state change by storing previous state and comparing after run). For production metrics, integrate with your monitoring in that wrapper.
