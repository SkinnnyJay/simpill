...
class IntervalManager {
  ...
  clearInterval(uniqueId: string): boolean {
    ...
    if (managed.type === "interval") clearInterval(managed.id);
  ...
  getStats(): IntervalStats {
    ...
    for (const managed of this.intervals.values()) {
      if (managed.type === "interval") activeIntervals++;
      else activeTimeouts++;
...
export function createTimerFactory(options: TimerFactoryOptions = {}): TimerFactory {
  const group = `factory_${++factoryCounter}_${options.group ?? "timers"}`;
  ...
  const ensureActive = (): void => {
    if (destroyed) {
      throw new Error("TimerFactory has been destroyed");
    }
  };

  const destroy = (_reason: "manual" | "idle-ttl" = "manual"): number => {
  ...
  return {
    createInterval: (name, callback, intervalMs, createOptions) => {
      ensureActive();
      scheduleIdleDestroy();
    ...
    createTimeout: (name, callback, timeoutMs, createOptions) => {
      ensureActive();
      scheduleIdleDestroy();
      const ttlMs = createOptions?.ttlMs ?? defaultTtlMs;
